...
class DonXinNghiViecUtils:
    ...
    def getTenCongty(text, mile):
        ...
        try:  
            ...
            indexEndLabel = text.lower().find(endLabel.lower(), indexStartLabel+len(startLabel), int(len(text)))
            if indexEndLabel > 0 :
                mile = int(indexEndLabel)
            result = text[int(indexStartLabel) + int(len(startLabel)): int(indexEndLabel)]
    ...
    def getTenPhongNhanSu(text, mile):
        ...
        try:  
            indexStartLabel = text.lower().find(startLabel.lower(),mile,len(text))
            indexEndLabel = text.lower().find(endLabel.lower(), indexStartLabel+len(startLabel),len(text))
            if indexEndLabel > 0 :
                mile = int(indexEndLabel)
            result = text[int(indexStartLabel) + int(len(startLabel)): int(indexEndLabel)]
        except Exception as e:
...
def donXinNghiViecMapper(raw_text):
    milestone = 0
    tenCongty, milestone = DonXinNghiViecUtils.getTenCongty(raw_text, milestone)
    tenPhongNhanSu, milestone = DonXinNghiViecUtils.getTenPhongNhanSu(raw_text, milestone)
    tenTruongPhong, milestone = DonXinNghiViecUtils.getTenTruongPhong(raw_text, milestone)
    ...
    thoiGianGanBo,  milestone = DonXinNghiViecUtils.getThoiGianGanBo(raw_text, milestone)
    kinhNghiem,  milestone = DonXinNghiViecUtils.getKinhNghiem(raw_text, milestone)
    nguoiBanGiao,  milestone = DonXinNghiViecUtils.getNguoiBanGiao(raw_text, milestone)
    boPhanBanGiao, milestone = DonXinNghiViecUtils.getBoPhanBanGiao(raw_text, milestone)
    congViecBanGiao,  milestone = DonXinNghiViecUtils.getCongViecBanGiao(raw_text, milestone)
    ngayVietDon, milestone = DonXinNghiViecUtils.getNgayVietDon(raw_text, milestone)
    specialCharacter = "[$@&?:]+"
